...
const Index = () => {
  ...
  if (isConfigured) {
    return <ChatInterface webhookUrl={webhookUrl} authHeader={Object.keys(authHeader).length ? authHeader : undefined} />;
  }
  return <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-center">Chat with your Webhook.</h1>
        
        <div className="space-y-4">
          <div>
            <label className="block mb-2 text-sm font-medium">Webhook URL</label>
            <Input value={webhookUrl} onChange={e => setWebhookUrl(e.target.value)} placeholder="Enter your Make.com webhook URL" />
          </div>
