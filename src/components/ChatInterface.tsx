  ...
  return <div className={`flex flex-col h-screen w-full shadow-lg ${isDark ? 'bg-gray-800 text-white' : 'bg-white'}`}>
      ...
      <div className="p-4 border-t flex flex-col gap-2">
        ...
        <div className="flex items-center space-x-2 w-full">
          <Input value={messageText} onChange={e => setMessageText(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className={`flex-grow ${isDark ? 'bg-gray-700 text-white border-gray-600' : ''} h-10 focus:h-10`} style={{
          minHeight: 40,
          height: 40,
          maxHeight: 40
        }} disabled={isLoading} />
          <Button onClick={handleSendMessage} disabled={isLoading || !messageText.trim() && attachments.length === 0} variant="default" size="icon" className="w-10 h-10 min-w-[44px] min-h-[40px] max-h-[40px] max-w-[44px] p-0 rounded-full flex items-center justify-center">
            <Send size={20} />
          </Button>
        </div>
      </div>
